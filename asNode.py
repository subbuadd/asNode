		...
		if yearCheck >= (nextYear):
			if (monthCheck >= nextMonth) or (yearCheck > nextYear):
				if (dayCheck >= nextDay) or (monthCheck > nextMonth):
					return False
					self._confirmAction('Error.. Error.. !!', True)
...
#		activList.getDependNode(0, mObj)
	...
	def listConstraints(self, conTypes=None):
		...
		#_ Else it list only requested type		
			asN.listConstraints(conType='point')  		
			asN.listConstraints(conType='parent')
		...
		#self.select(r=1)
		conNodes =[]
		conList =['point', 'orient', 'parent', 'scale', 'aim', 'geometry', 'normal', 'tangent'] #, 'poleVector']		
		if conTypes:
			conTypes =[conTypes] if type(conTypes) != list else conTypes 				
			for conType in conTypes:
				if conType not in conList:
					eRig.error("conType should be one of the list['point', 'orient', 'parent', 'scale', 'aim', 'geometry', 'normal', 'poleVector']")
				
				conNode =None 
				exec ('conNode = ' + conType + 'Constraint(\"' + self.name() + '\", q=1)')
				if conNode:	
					conNodes.append(asNode(conNode))			
		else:
	...
	def connectAttr(self, srcAttr, trgtObj, trgtAttr=None, **kwargs):
		...
		for trgtObj in trgtList:
			if '.' in str(trgtObj):
				try:
					connectAttr(self.attr(srcAttr), trgtObj, f=1)
				except:
					self._confirmAction('Connection can not be made ..!', raiseErr=True)
			else:
				if trgtAttr:
					connectAttr(self.attr(srcAttr), trgtObj + '.' + trgtAttr, f=1)
				else:
	...
	def disconnectAttr(self, srcAttr, trgtObj=None, trgtAttr=None):
		if '.' in str(trgtObj):
			disconnectAttr(self.attr(srcAttr), trgtObj)
		else:
			if trgtAttr:
				disconnectAttr(self.attr(srcAttr), trgtObj + '.' + trgtAttr)
			else:
	...
	def distanceTo(self, destObjOrPos, getNode=False):
		...
		#_ Get source position			
		srcObj =self.name()
		srcPos =self.getPos()    		
					
		#_ Get destination position	
		destNode =None
		...
		#_ Calculating mDistance between srcPos and destPos			
		distX =destPos[0] - srcPos[0]
		distY =destPos[1] - srcPos[1]
		distZ =destPos[2] - srcPos[2]				
		mDist =math.sqrt(distX**2 + distY**2 + distZ**2)
		
		#_ Return Distance Node, if getNode is True
		if getNode:
			#distShape =distanceDimension(sp=(srcPos[0], srcPos[1], srcPos[2]), ep=(destPos[0], destPos[1], destPos[2]))
			distShape =asNode(distanceDimension(sp=(-100, -100, -100), ep=(100, 100, 100)))
			distNode =asNode(distShape.parent())			
			distNode =distNode.rename(destNode.shortName() + '_Dist')
			distAttr =distShape.name() + '.distance'
			srcLoc =asNode(listConnections(distShape.name() + '.startPoint')[0])
			destLoc =asNode(listConnections(distShape.name() + '.endPoint')[0])
